        ...
        >
          ...
          <div class="flex items-start relative group">
            ...
            >
              ...
              <!-- Сообщение с резюме (отображается ПОД основным текстом) -->
              <resume-message-card
                v-if="isResumeMessage(message.content)"
                :resume="parseResumeMessage(message.content)"
                class="mt-2"
                @view-resume="
                  $emit('view-resume', parseResumeMessage(message.content))
                "
                @view-pdf="viewPdf"
              />
  ...
  setup(props) {
    ...
    const isResumeMessage = (content) => {
      ...
      try {
        ...
        return false; // Если не JSON, это не сообщение с резюме
      }
    };
    ...
    // Добавляем метод для просмотра PDF файла
    const viewPdf = async (resumeId) => {
      try {
        console.log("ChatWindow: Просмотр PDF для резюме ID:", resumeId);
        ...
        // Улучшенный алгоритм поиска URL резюме в сообщениях
        let pdfUrl = null;

        // Ищем во всех сообщениях сообщение с информацией о резюме
        for (const msg of props.messages) {
          try {
            if (msg.content) {
              ...
              if (
                data &&
                data.type === "resume_attached" &&
                data.resume_id &&
                String(data.resume_id) === String(resumeId) &&
                data.resume_file_url
              ) {
                pdfUrl = data.resume_file_url;
                console.log("ChatWindow: Найден URL PDF в сообщении:", pdfUrl);
                break;
              }
            }
          } catch (e) {
        ...
        // Если ссылки нет в сообщениях, запрашиваем данные резюме через API
        if (!pdfUrl) {
          console.log(
            "ChatWindow: Запрашиваем PDF через API для ID:",
            resumeId
          );

          // Используем общедоступный API эндпоинт для всех пользователей
          try {
            console.log(
              "ChatWindow: Используем стандартный API для всех пользователей"
            );
            const response = await axios.get(`/api/resumes/${resumeId}/`);
            if (response.data && response.data.resume_file_url) {
              pdfUrl = response.data.resume_file_url;
              console.log(
                "ChatWindow: Получен URL PDF из стандартного API:",
                pdfUrl
              );
            }
          } catch (apiError) {
            console.error(
              "ChatWindow: Ошибка стандартного API при получении PDF:",
              apiError
            );

            // Пробуем альтернативный эндпоинт для публичных резюме
            try {
              console.log("ChatWindow: Пробуем API публичных резюме");
              ...
              if (
                ...
                fileResponse.data[0].resume_file_url
              ) {
                pdfUrl = fileResponse.data[0].resume_file_url;
                console.log(
                  "ChatWindow: Получен URL PDF из API публичных резюме:",
                  pdfUrl
                );
              }
            } catch (fileError) {
              console.error(
                "ChatWindow: Ошибка при получении из публичных резюме:",
                fileError
              );
            }
          }
        ...
        if (pdfUrl) {
          ...
          const pdfUrlWithTimestamp = `${pdfUrl}?t=${new Date().getTime()}`;

          // Используем getFileUrl для преобразования localhost в IP-адрес
          const fullPdfUrl = getFileUrl(pdfUrlWithTimestamp);
          console.log("ChatWindow: Полный URL PDF:", fullPdfUrl);
          ...
          if (isMobile) {
            console.log(
              "ChatWindow: Обнаружено мобильное устройство, открываем PDF"
            );

            // Прямой и надежный метод для мобильных - смена window.location
            window.location.href = fullPdfUrl;
            console.log("ChatWindow: Перенаправление на PDF URL:", fullPdfUrl);
          } else {
            ...
            if (
              ...
              console.log(
                "ChatWindow: Открытие окна заблокировано, пробуем альтернативный метод"
              );
          ...
          console.error("ChatWindow: URL PDF не найден");
          showNotification(
            "PDF файл не найден. Попробуйте просмотреть полное резюме или свяжитесь с соискателем.",
            "warning"
          );
        }
      } catch (error) {
        console.error("ChatWindow: Ошибка при просмотре PDF:", error);
        showNotification("Произошла ошибка при открытии PDF файла.", "error");
      }
    };
    ...
    const formatMessage = (text) => {
      ...
      // Если это JSON с резюме, извлекаем из него текст
      try {
        const data = JSON.parse(text);
        if (data && data.type === "resume_attached") {
          // Если у нас есть поле message, используем его
          if (data.message) {
            return data.message;
          }
          // Иначе используем сформированное сообщение об отклике
          else {
            return `Здравствуйте, я откликнулся на вакансию ${
              data.job_title || ""
            }`;
          }
        }
