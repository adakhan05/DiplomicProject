<template>
  <div class="min-h-screen bg-gray-900 text-white">
    ...
    <router-link
      v-if="
        isAuthenticated && !(route.path.includes('/messages') && isMobileView)
      "
      to="/messages"
      class="fixed bottom-5 right-5 inline-flex items-center justify-center w-14 h-14 bg-green-500 text-white rounded-full hover:bg-green-600 shadow-lg z-40 message-icon"
    >
...
html {
  visibility: hidden;
  overscroll-behavior: none; /* Отключаем скролл на странице */
}
...
* {
  ...
  scrollbar-color: rgba(75, 85, 99, 0.5) rgba(31, 41, 55, 0.5); /* Цвет скроллбара */
}
...
/* Отключаем скролл на модальных окнах */
