...
onMounted(async () => {
  ...
  if (route.params.id) {
    ...
    try {
      // Получение данных вакансии через store
      const jobId = route.params.id;
      let jobData = null;

      try {
        // Попытка получить данные вакансии через store
        jobData = await jobStore.getJob(jobId);
        console.log("Получены данные вакансии:", jobData);
      } catch (storeError) {
        console.error("Ошибка при загрузке вакансии через store:", storeError);

        // Попытаемся получить из localStorage как запасной вариант
        try {
          const localJobs = JSON.parse(localStorage.getItem("jobs") || "[]");
          jobData = localJobs.find((job) => Number(job.id) === Number(jobId));
          if (jobData) {
            console.log("Данные вакансии получены из localStorage:", jobData);
          }
        } catch (localError) {
          console.error(
            "Ошибка при загрузке вакансии из localStorage:",
            localError
          );
        }
      }
      ...
      if (jobData) {
        ...
        form.value = getDefaultFormValues();
        errors.value.general =
          "Не удалось найти данные вакансии. Используются значения по умолчанию.";
      }
      ...
      errors.value.general = `Не удалось загрузить данные вакансии: ${error.message}`;
    } finally {
      loading.value = false;
    }
  }
...
const populateFormWithJobData = (jobData) => {
  form.value = {
    ...
    experience: jobData.experience || "no_experience",
    location: jobData.location || "",
    skills: jobData.skills?.slice() || [],
