<template>
  ...
  <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <!-- Add a visibility toggle before the form buttons -->
...
const handleSubmit = async () => {
  try {
    ...
    if (import.meta.env.DEV) {
      ...
      try {
        // Получение существующих вакансий из localStorage
        const localJobs = JSON.parse(localStorage.getItem("jobs") || "[]");

        if (isEditing.value) {
          // Обновление существующей вакансии
          const jobIndex = localJobs.findIndex(
            (job) => Number(job.id) === Number(route.params.id)
          );
          ...
          if (jobIndex !== -1) {
            ...
            localJobs[jobIndex] = createdJob;
            console.log(
              "РЕЖИМ РАЗРАБОТКИ: Вакансия обновлена в localStorage:",
              createdJob
            );
          } else {
            console.error(
              `РЕЖИМ РАЗРАБОТКИ: Вакансия с ID ${route.params.id} не найдена для редактирования`
            );
          }
        } else {
          ...
          const newJobId =
            localJobs.length > 0
              ? Math.max(...localJobs.map((j) => Number(j.id))) + 1
              : 1;
          ...
          localJobs.push(createdJob);
          console.log(
            "РЕЖИМ РАЗРАБОТКИ: Новая вакансия создана в localStorage:",
            createdJob
          );
        }
        ...
        // Установка флага, указывающего, что вакансия была недавно создана/обновлена
        // Это помогает списку вакансий узнать, что нужно обновить данные
        localStorage.setItem("recently_created_job", "true");

        // Обновление хранилища вакансий
        if (isEditing.value) {
          const storeIndex = jobStore.jobs.findIndex(
            (job) => Number(job.id) === Number(route.params.id)
          );
          if (storeIndex !== -1) {
            jobStore.jobs[storeIndex] = createdJob;
          }
        } else {
          jobStore.jobs.push(createdJob);
        }
        ...
        console.error(
          "РЕЖИМ РАЗРАБОТКИ: Ошибка обновления localStorage:",
          localStorageError
        );
      }
    }
    ...
    if (!import.meta.env.DEV || !createdJob) {
      if (isEditing.value) {
        ...
        console.log("Ответ обновления вакансии:", response.data);
        createdJob = response.data;
      } else {
        console.log("Создание новой вакансии");
        const response = await axios.post("/api/jobs/", jobData);
        console.log("Ответ создания вакансии:", response.data);
        createdJob = response.data;

        // Удостовериться, что вакансия активна, установив это явно
        if (response.data && response.data.id && form.value.is_active) {
          try {
            await axios.post(`/api/jobs/${response.data.id}/toggle-active/`, {
              is_active: true,
            });
            console.log("Вакансия успешно активирована");
          } catch (activateErr) {
            console.error("Ошибка активации вакансии:", activateErr);
          }
        }
      }
    }

    // Переход напрямую вместо использования роутера
