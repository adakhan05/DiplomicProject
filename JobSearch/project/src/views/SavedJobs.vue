              ...
              <div class="p-6">
                  ...
                  <div class="mb-3 pr-10">
                    ...
                    <div class="flex flex-wrap items-center text-gray-300 mb-2">
                      <span class="mr-3 font-medium">
                        <template
                          v-if="savedJob.job.id === 1 || savedJob.job.id === 2"
                        >
                          TrueTechStroy
                        </template>
                        <template v-else>
                ...
                <div class="space-y-3 my-3">
                  <div class="flex flex-wrap gap-2">
                    <span
                      v-if="savedJob.job.id === 1"
                      class="inline-block bg-gray-700 text-green-500 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      Полный день
                    </span>
                    <span
                      v-else-if="savedJob.job.id === 2"
                      class="inline-block bg-gray-700 text-green-500 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      Удаленная работа
                    </span>
                    <span
                      v-else-if="savedJob.job.employment_type === 'full_time'"
                      class="inline-block bg-gray-700 text-green-500 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      Полный день
                    </span>
                    ...
                    <span
                      v-if="
                        ...
                        savedJob.job.experience_level.includes('Без опыта')
                      "
                    ...
                    <span
                      v-else-if="
                        savedJob.job.experience_level &&
                        savedJob.job.experience_level.includes('1-3')
                      "
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      1-3 года
                    ...
                    <span
                      v-else-if="
                        savedJob.job.experience_level &&
                        savedJob.job.experience_level.includes('3-5')
                      "
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      3-5 лет
                    ...
                    <span
                      v-else-if="
                        savedJob.job.experience_level &&
                        savedJob.job.experience_level.includes('Более 5')
                      "
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      Более 5 лет
                    ...
                    <span
                      v-else-if="savedJob.job.experience_level"
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      {{ savedJob.job.experience_level }}
                    </span>
                    <span
                      v-else-if="savedJob.job.id === 1"
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
                      Без опыта
                    </span>
                    ...
                    <span
                      v-else-if="savedJob.job.experience_years"
                      class="inline-block bg-gray-700 text-blue-400 text-xs font-semibold px-3 py-1 rounded-full"
                    >
...
export default {
  setup() {
    ...
    const filteredJobs = computed(() => {
      ...
      if (filters.experienceLevel.length > 0) {
        result = result.filter(
          (saved) =>
            saved.job &&
            saved.job.experience_level &&
            filters.experienceLevel.includes(saved.job.experience_level)
        );
      }
      ...
      if (filters.minSalary) {
        result = result.filter(
          (saved) =>
            saved.job &&
            (!saved.job.min_salary || saved.job.min_salary >= filters.minSalary)
        );
      }
      ...
      if (filters.maxSalary) {
        result = result.filter(
          (saved) =>
            saved.job &&
            (!saved.job.max_salary || saved.job.max_salary <= filters.maxSalary)
        );
      }
    ...
    const loadSavedJobs = async () => {
      ...
      try {
        console.log("SavedJobs.vue: Загрузка сохраненных вакансий");

        await savedJobsStore.fetchSavedJobs();

        console.log(
          `SavedJobs.vue: Загружено ${savedJobsStore.savedJobs.length} вакансий из хранилища`
        );
    ...
    const removeJob = async (savedJob) => {
      if (!savedJob || !savedJob.job) {
        console.error("Invalid job data for removal in component");
        return;
      }
      const jobId = Number(savedJob.job.id);
      console.log(`Requesting removal of job ID=${jobId} from component`);
    ...
    const calculateStats = () => {
      ...
      savedJobs.value.forEach((saved) => {
        ...
        if (saved?.job?.experience_level) {
          experienceStats[saved.job.experience_level] =
            (experienceStats[saved.job.experience_level] || 0) + 1;
        }
      });
    ...
    const applyToJob = (savedJob) => {
      ...
      const clickedState = reactive({});
      clickedState[savedJob.job.id] = true;
      console.log(`Applied to job: ${savedJob.job.id} - ${savedJob.job.title}`);

      setTimeout(() => {
        delete clickedState[savedJob.job.id];
      }, 3000);
    };
    ...
    const formatExperienceYears = (years) => {
      ...
      if (years < 2) return "1 год";
      if (years >= 2 && years < 5) return `${years.toFixed(0)} года`;
      return `${years.toFixed(0)} лет`;
    };
