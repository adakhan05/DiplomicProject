  ...
  actions: {
    ...
    async sendMessage(messageData) {
      ...
      try {
        ...
        // Сначала добавляем временное сообщение в локальный кэш для мгновенного отображения
        const conversationId = messageData.conversation_id;

        // Создаем временное сообщение со свойством is_pending
        const tempMessage = {
          id: `temp_${Date.now()}`,
          ...
          is_read: false,
          is_pending: true, // Отметка, что сообщение ожидает отправки
        };
        ...
        // Добавляем временное сообщение в массив сообщений
        ...
        if (!successMessageUrl) {
          // Находим временное сообщение и помечаем его как ошибочное
          const index = this.messages[conversationId].findIndex(
            (m) => m.id === tempMessage.id
          );
          if (index !== -1) {
            this.messages[conversationId][index].is_pending = false;
            this.messages[conversationId][index].has_error = true;
          }
        ...
        const tempIndex = this.messages[conversationId].findIndex(
          (m) => m.id === tempMessage.id
        );
        if (tempIndex !== -1) {
          this.messages[conversationId][tempIndex] = sentMessage;
        } else {
    ...
    handleWebSocketMessage(data) {
      ...
      try {
        ...
        const strConversationId = String(conversationId); // Убедимся, что это строка
        const message = {
          id: data.message_id || Date.now(), // ID самого сообщения (используем timestamp если ID нет)
          conversation_id: strConversationId, // ID диалога, к которому оно относится
          ...
          sender_name: data.sender_name || "Пользователь", // Имя отправителя (строка)
          content: data.message, // Текст сообщения
          created_at: data.created_at || new Date().toISOString(), // Время создания
          is_read: false, // Новые сообщения по умолчанию не прочитаны
          is_new: true, // Помечаем как новое сообщение для прокрутки
        };

        console.log(
          `[DEBUG] chatStore.handleWebSocketMessage: Сформированный объект сообщения для диалога ${strConversationId}:`,
          message
        );

        // Создаем массив сообщений для диалога, если его еще нет
        if (!this.messages[strConversationId]) {
          this.messages[strConversationId] = [];
        }

        // Проверяем, нет ли уже этого сообщения в массиве
        const existingMessageIndex = this.messages[strConversationId].findIndex(
          (m) => m.id === message.id
        );

        if (existingMessageIndex === -1) {
          // Добавляем новое сообщение в массив
          this.messages[strConversationId].push(message);

          // Сортируем сообщения по времени создания
          this.messages[strConversationId].sort(
            (a, b) => new Date(a.created_at) - new Date(b.created_at)
          );

          console.log(
            `[DEBUG] chatStore.handleWebSocketMessage: Сообщение добавлено в диалог ${strConversationId}`
          );

          // Обновляем информацию о последнем сообщении в списке диалогов
          this.updateConversationLastMessage(strConversationId, message);
          ...
          if (String(data.sender_id) !== String(authStore.user?.id)) {
            ...
            // Обновляем счетчик непрочитанных сообщений в конкретном диалоге
            const conversationIndex = this.conversations.findIndex(
              (c) => String(c.id) === strConversationId
            );
            if (conversationIndex !== -1) {
              this.conversations[conversationIndex].unread_count =
                (this.conversations[conversationIndex].unread_count || 0) + 1;
          ...
          // Устанавливаем флаг, что получено новое сообщение
          // Это поможет компонентам узнать, что нужно прокрутить чат вниз
          this.newMessageReceived = true;

          // Через короткое время сбрасываем флаг
          setTimeout(() => {
            this.newMessageReceived = false;
          }, 1000);
        } else {
          console.log(
            `[DEBUG] chatStore.handleWebSocketMessage: Сообщение с ID ${message.id} уже существует в диалоге ${strConversationId}`
          );
        }
      } catch (error) {
        console.error(
          "[DEBUG] chatStore.handleWebSocketMessage: Ошибка при обработке сообщения:",
          error
        );
      }
    },
